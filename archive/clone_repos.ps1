$sourcePath = "skill-manage\src\source.md"
...
$existingUrls = @{}
$subdirs = Get-ChildItem "skill-manage\src" -Directory
...
foreach ($url in $urlsToClone.Keys) {
    if (-not $existingUrls.ContainsKey($url)) {
        ...
        if (-not (Test-Path "skill-manage\src\$targetDir")) {
            Write-Host "Cloning $url into $targetDir"
            $gitUrl = "$url.git"
            git clone $gitUrl "skill-manage\src\$targetDir"
            $cloned++
        } else {
