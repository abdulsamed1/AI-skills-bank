...
* **Opening Scene:** Omar has just added 50 new repository URLs to the `.skills-bank-cli-config.json` manifest. Now he needs to regenerate the massive `hub-manifests.csv` and all sub-hub index files.
...
* **Local State:** Rely on `.skills-bank-cli-config.json` and the `lib/` directory relative to the current working directory for local project execution.
...
* **FR1:** The system can read a local manifest file (e.g., `.skills-bank-cli-config.json`) containing a list of remote repository URLs.
